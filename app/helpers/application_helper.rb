...
module ApplicationHelper
  ...
  def render_facet_value(facet_solr_field, item, options ={})
    link_to_unless(options[:suppress_link], item.value, add_facet_params_and_redirect(facet_solr_field, item.value), :class=>"facet_select") + " (" + format_num(item.hits) + ")" + render_subfacets(facet_solr_field, item, options)
  end
  ...
  def render_selected_facet_value(facet_solr_field, item)
    '<span class="selected">' +
    link_to_unless(true, item.value, add_facet_params_and_redirect(facet_solr_field, item.value), :class=>"facet_select") + " (" + format_num(item.hits) + ")" +
    '</span>' +
